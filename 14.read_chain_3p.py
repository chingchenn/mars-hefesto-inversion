...
def plot_trace(chains, output_dir, prefix=''):
...
def plot_autocorr(trimmed_chains, output_dir, prefix=''):
    n_chains = len(trimmed_chains)
    fig, axes = plt.subplots(2, 2, figsize=(10, 7))
    fig.suptitle('Autocorrelation (post Burn-in)',
    ...
    for idx, p in enumerate(PARAMS):
        ...
        ax.bar(range(maxlag), ac, color='darkblue', alpha=0.7, width=1.0)
        ax.axhline(0,    color='black', lw=0.8)
        ax.axhline(0.05, color='red',   ls='--', lw=0.8)

        all_samples = np.concatenate([ch['params'][p] for ch in trimmed_chains])
...
def plot_marginal_posteriors(samples, misfits, output_dir, prefix=''):
...
def plot_corner(samples, output_dir, max_samples=5000, prefix=''):
    n   = len(PARAMS)
...
def plot_misfit_vs_params(samples, misfits, output_dir, prefix=''):
    fig, axes = plt.subplots(1, 4, figsize=(16, 4))
    ...
    plt.tight_layout()
    out = os.path.join(output_dir, f'{prefix}_05_misfit_vs_params.png')
    plt.savefig(out, dpi=150, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    # Trace plots
    print("\n[1] Trace plots")
    plot_trace(chains, output_dir,prefix=args.prefix)
    ...
    # Autocorrelation
    print("\n[3]  Autocorrelation")
    plot_autocorr(trimmed, output_dir,prefix=args.prefix)
    ...
    plot_marginal_posteriors(samples, misfits, output_dir,prefix=args.prefix)

    # Corner plot
    print("\n[6] plot_corner")
    plot_corner(samples, output_dir,prefix=args.prefix)

    # Misfit vs parameters
    print("\n[7] plot_misfit_vs_params")
    plot_misfit_vs_params(samples, misfits, output_dir,prefix=args.prefix)
